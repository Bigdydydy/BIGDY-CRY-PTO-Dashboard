 ...
 * Supports direct Google Gemini API (gemini-2.5-flash / gemini-1.5-flash)
 ...
 * combining live market metrics with the specific post context and author pedigree.
 */
function generateHeuristicQuantAnalysis(post, promptType, customQuestion) {
  ...
  if (promptType === 'macro_logic') {
    return `### 📊 宏观逻辑穿透报告：${author} 核心论点解构

**1. 底层宏观假设与传导路径**
- **核心论点**：${author} 在该推文中指出：*“${text.slice(0, 100)}...”*。该观点的核心锚点在于宏观微观管道的脱节与利差结构的分化。
- **资产负债表传导机制**：
...
- **潜在爆仓与挤压区**：
  ...
  - 上行真空区：突破主要阻力壁垒后，做市商将由正 Gamma 翻转为负 Gamma，可能触发空头回补的 Gamma Squeeze。
...
- 则本论点所构建的宽松与避险逻辑即告失效，应立即降低多头敞口。`;
  ...
  return `### ✦ Gemini 宏观量化研判：针对用户提问的穿透解析
...
Answer with institutional-grade quantitative finance precision.`;
  ...
  if (effectiveApiKey) {
    try {
      const liveAnalysis = await callGeminiApi(promptText, effectiveApiKey, 'gemini-2.5-flash');
      return {
        ok: true,
        source: 'gemini-api',
  ...
  const analysis = generateHeuristicQuantAnalysis(post, promptType, customQuestion);
  return {
    ok: true,
    source: 'local-quant-engine',
    model: 'Institutional Quant Heuristic Copilot',
    analysis,
    promptType,
    customQuestion,
    latencyMs: Date.now() - startTime,
